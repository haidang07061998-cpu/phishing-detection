...
- **Audit actor từ key, không từ body**: các write endpoints (`/whitelist` POST/DELETE, `/threat` POST/DELETE, `/keys` POST/DELETE) dùng helper `_actor()` trong `app.py` để ghi `added_by`/`removed_by`/`created_by` — identity chỉ lấy từ `request.api_key` đã verify bởi `require_api_key` (name + key_id). Các field `added_by`/`removed_by` gửi trong request body bị **bỏ qua** → client không thể giả mạo người thao tác trong audit log.
