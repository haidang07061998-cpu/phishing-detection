...
function ReportsPanel() {
  ...
  const load = useCallback(async (key) => {
    ...
    try {
      ...
      if (hist && Array.isArray(hist.records)) {
        ...
        setError(friendlyError({ status: 403, message: hist.error }, t))
      }
  ...
  const exportData = async (fmt) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(d.error || t('reports.exportFailed'))
        return
      }
  ...
  const addThreat = async (e) => {
    ...
    if (!value) return
    if (!adminKey.trim()) {
      setError(t('reports.adminKeyRequired'))
      return
    }
    try {
      const apiUrl = getApiUrl()
      ...
      if (!res.ok) {
        const d = await res.json().catch(() => ({}))
        setError(d.error || t('reports.addFailed'))
        return
      }
      setNewThreat('')
      load(adminKey.trim())
    ...
    <div style={{ width: '100%', marginTop: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '220px' }}>
            ...
            <input
              ...
              placeholder={t('reports.keyPlaceholder')} aria-label={t('reports.keyAria')}
              autoComplete="off"
          ...
          <button type="submit" disabled={!adminKey.trim()} style={{
            ...
            {keyEntered ? t('reports.connected') : t('reports.load')}
          </button>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem' }}>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '0.9rem 1rem' }}>
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('reports.total')}</p>
          <p style={{ margin: '0.3rem 0 0', color: 'var(--text-bright)', fontSize: '1.3rem', fontWeight: 700 }}>{summary?.total ?? '–'}</p>
        </div>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '0.9rem 1rem' }}>
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('reports.phishing')}</p>
          <p style={{ margin: '0.3rem 0 0', color: 'var(--danger)', fontSize: '1.3rem', fontWeight: 700 }}>{summary?.counts?.phishing ?? '–'}</p>
        </div>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '0.9rem 1rem' }}>
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('reports.suspicious')}</p>
          <p style={{ margin: '0.3rem 0 0', color: 'var(--warning)', fontSize: '1.3rem', fontWeight: 700 }}>{summary?.counts?.suspicious ?? '–'}</p>
        </div>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '0.9rem 1rem' }}>
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('reports.threatHits')}</p>
          <p style={{ margin: '0.3rem 0 0', color: 'var(--accent)', fontSize: '1.3rem', fontWeight: 700 }}>{summary?.threat_db_hits ?? '–'}</p>
        </div>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '0.9rem 1rem' }}>
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('reports.blocklist')}</p>
      ...
      {error && (
        <div role="alert" style={{ background: 'var(--danger-bg)', border: '1px solid var(--danger)', borderRadius: '10px', padding: '0.75rem 1rem' }}>
          <p style={{ margin: 0, color: 'var(--danger)', fontSize: '0.8rem', fontWeight: 600 }}>{t('reports.unavailable')}</p>
          <p style={{ margin: '0.2rem 0 0', color: 'var(--danger)', fontSize: '0.75rem' }}>{t('reports.unavailableHint', { error })}</p>
        </div>
      )}

      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '1.25rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.75rem' }}>
          <h3 style={{ margin: 0, fontSize: '0.95rem', color: 'var(--text-bright)', fontWeight: 600 }}>{t('reports.history')}</h3>
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            <input
              type="text" value={filter} onChange={(e) => setFilter(e.target.value)}
              placeholder={t('reports.filter')} aria-label={t('reports.filter')}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.75rem' }}>
              <thead>
                <tr style={{ color: 'var(--text-muted)', textAlign: 'left' }}>
                  <th style={{ padding: '0.35rem 0.5rem', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>{t('reports.col.time')}</th>
                  <th style={{ padding: '0.35rem 0.5rem', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>{t('reports.col.target')}</th>
                  <th style={{ padding: '0.35rem 0.5rem', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>{t('reports.col.verdict')}</th>
                  <th style={{ padding: '0.35rem 0.5rem', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>{t('reports.col.score')}</th>
                  <th style={{ padding: '0.35rem 0.5rem', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>{t('reports.col.latency')}</th>
                  <th style={{ padding: '0.35rem 0.5rem', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>{t('reports.col.quality')}</th>
                  <th style={{ padding: '0.35rem 0.5rem', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>{t('reports.col.threatDb')}</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((r, i) => (
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '1.25rem' }}>
        ...
        <form onSubmit={addThreat} style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem' }}>
          <input
            ...
            placeholder={t('reports.threatPlaceholder')} aria-label={t('reports.threatPlaceholder')}
          ...
          />
          <button type="submit" disabled={!newThreat.trim()} style={{ padding: '0.4rem 0.9rem', fontSize: '0.78rem', borderRadius: '6px', border: 'none', background: 'var(--danger)', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>{t('reports.add')}</button>
        </form>
        {localThreats.length === 0 ? (
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.78rem' }}>
            {t('reports.threatEmpty')}
          </p>
        ) : (
